...
"""
script for successful bundles only
...
CSV_FILE = "/media/Data/csv/after_attack_dynamics.csv"
...
df_included_bundles = df_bundles.loc[df_bundles["includedInBlock"]==1]
start_block = int(df_included_bundles["blockNumber"].min()) - 1
df_sandwich_swap_attack_transactions["bundleId"] = np.int64(df_sandwich_swap_attack_transactions["bundleId"])
df_included_attack_transactions = df_sandwich_swap_attack_transactions.loc[df_sandwich_swap_attack_transactions["bundleId"].isin(df_included_bundles.index)]
df_included_attack_components = df_sandwich_swap_attack_components.loc[df_sandwich_swap_attack_components["sandwichSwapAttackTransactionId"].isin(df_included_attack_transactions.index)]
df_included_attack_components = df_included_attack_components.copy()
...
MAX_HISTORY_SECONDS = 12 * 1001

df_included_attack_components["blockNumber"] = np.nan
...
while block_number <= last_block:
    try:
        ...
        df_current_block_sandwiches = df_included_attack_components.loc[(df_included_attack_components["createTime"] <= block_time + timedelta(seconds=12))
                                                                             & (df_included_attack_components["createTime"] >= block_time - timedelta(seconds=MAX_HISTORY_SECONDS))
                                                                             & (df_included_attack_components["position"] == "front-run")]
        for i in df_current_block_sandwiches.index:
            ...
            if np.isnan(df_included_attack_components.loc[i, "blockNumber"]):
                df_included_attack_components.loc[i, "blockNumber"] = block_number
                first_block_for_this_sandwich = block_number
            else:
                first_block_for_this_sandwich = df_included_attack_components.loc[i, "blockNumber"]
...
t = "0x01eF8cC6200f2D88Bd7b9Ca6c0E6131A19458E06"
for p in pair_storage:
    if pair_storage[p]["token0"] == t or pair_storage[p]["token1"] == t:
        print(p, token_storage[t])
        break
